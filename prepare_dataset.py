...
def main():
    ...
    # 1. Gather all generated image files
    valid_exts = {".jpg", ".jpeg", ".png"}
    image_files = [f for f in os.listdir(gen_dir) if os.path.splitext(f)[1].lower() in valid_exts]

    print(f"🔍 Inspecting {len(image_files)} base synthetic images for quality & anatomy...")
    ...
    for idx, filename in enumerate(image_files):
        ...
        # Generate tentative annotation to test MediaPipe confidence & landmark geometry
        ...
        if (idx + 1) % 200 == 0 or (idx + 1) == len(image_files):
            print(f"  ✓ Processed {idx+1}/{len(image_files)} base images...")

    print(f"✨ Quality Filter Complete: {len(clean_samples)} Clean Passed | {rejected_count} Rejected!")
    ...
    print("\n📦 Saving Train, Validation, and Test Dataset Splits...")
    ...
    print(f"\n📊 Dataset preparation complete! Summary report saved to '{report_path}'!")
